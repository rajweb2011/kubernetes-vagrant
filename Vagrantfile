  # I have tested this code to run on 2 cpus and 3048 gb of ram please change according to your machine capacity 
  # options are documented and commented below. For a complete reference,
  # please contact rajesh for any errors 
Vagrant.configure("2") do |config|
  config.vm.provision :shell, privileged: true, inline: $install_common_tools
  # declaring master node for kubernetes 
  ...
  # iteration of nodes which is given above  
  %w{node1 node2 node3}.each_with_index do |name, i|
    config.vm.define name do |node|
      node.vm.provider "virtualbox" do |vb|
        vb.name = "node#{i + 1}"
          # please  change as per your machine capacity 
        vb.memory = 2048
        vb.cpus = 2
      end
...
echo "10.0.0.20 master" >> /etc/hosts
echo "10.0.0.21 node1" >> /etc/hosts
echo "10.0.0.22 node2" >> /etc/hosts
echo "10.0.0.23 node3" >> /etc/hosts
# disable swap
swapoff -a
...
# Create rsa key and pusht id_rsa
ssh-keygen -q -t rsa -b 4096 -N '' -f /home/vagrant/.ssh/id_rsa
cat /home/vagrant/.ssh/id_rsa.pub >> /home/vagrant/.ssh/authorized_keys
cat /home/vagrant/.ssh/id_rsa.pub > ${KEY_FILE}

# Start cluster
sudo kubeadm init --apiserver-advertise-address=10.0.0.20 --pod-network-cidr=10.244.0.0/16 | grep -Ei "kubeadm join|discovery-token-ca-cert-hash" > ${OUTPUT_FILE}
chmod +x $OUTPUT_FILE
...
# Fix kubelet IP
echo 'Environment="KUBELET_EXTRA_ARGS=--node-ip=10.0.0.20"' | sudo tee -a /etc/systemd/system/kubelet.service.d/10-kubeadm.conf
